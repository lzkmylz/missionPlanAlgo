...
from core.models.pmc_config import PitchMotionCompensationConfig, PMC_CONFIG_25PERCENT
...
def demo_pmc_performance():
    ...
    for reduction in [0.10, 0.25, 0.50, 0.75]:
        params = calculate_pmc_parameters(orbit_altitude, reduction)
        print(f"{reduction*100:.0f}%{'':<7} "
              f"{params['pitch_rate_dps']:.3f}{'':<10} "
              f"{params['snr_gain_db']:.2f}{'':<10} "
...
def demo_pmc_payload_config():
    ...
    # 创建支持PMC的载荷配置
    from core.models.imaging_mode import OPTICAL_PUSH_BROOM_HIGH_RES, OPTICAL_PMC_25PERCENT
