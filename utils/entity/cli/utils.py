...
Note: JSON utilities have been moved to utils.json_utils.
...
# Import base functions from shared module
try:
...
def safe_load_json(file_path: Union[str, Path], allowed_base_dir: Optional[str] = None) -> Any:
    ...
    # Use the base load_json function for actual file reading
    try:
...
def safe_save_json(data: Any, file_path: Union[str, Path], indent: int = 2) -> None:
    ...
    """
    try:
        _save_json(data, file_path, indent)
    except TypeError as e:
        raise click.ClickException(f"Cannot serialize data to JSON: {str(e)}")
    except ValueError as e:
        raise click.ClickException(f"Cannot serialize data to JSON: {str(e)}")
    except PermissionError:
        raise click.ClickException(f"Permission denied: Cannot write to {file_path}")
    except OSError as e:
        raise click.ClickException(f"Cannot write to {file_path}: {str(e)}")
