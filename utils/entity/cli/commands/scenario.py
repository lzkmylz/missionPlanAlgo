...
def add_to_scenario(scenario_path: str, entity_type: str, template: Optional[str],
    ...
    if from_file:
        ...
        if entity_type == "satellite":
            ...
            if not template:
                raise click.BadParameter("--template is required for ground stations")

            scenario = builder.add_ground_station_to_scenario(
                scenario, template, entity_id
            )

            if dry_run:
                _print_dry_run_changes(original_scenario, scenario, f"ground_station {entity_id}")
            else:
                builder.save_scenario(scenario, scenario_path)
                click.echo(f"Added ground station '{entity_id}' to scenario.")

        elif entity_type == "target":
            if not template:
                raise click.BadParameter("--template is required for targets")

            scenario = builder.add_target_to_scenario(
                scenario, template, entity_id
            )

            if dry_run:
                _print_dry_run_changes(original_scenario, scenario, f"target {entity_id}")
            else:
                builder.save_scenario(scenario, scenario_path)
                click.echo(f"Added target '{entity_id}' to scenario.")
